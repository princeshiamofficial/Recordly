	...
	return {
		...
		detail:
			normalizedProgress >= 100
				? "Finishing the update download. CamVerse will restart as soon as the installer is ready."
				: remainingMb !== null
					? `${remainingMb.toFixed(1)} MB left before CamVerse restarts.`
					: "Downloading the update now. CamVerse will restart when it finishes.",
		delayMs: UPDATE_REMINDER_DELAY_MS,
...
async function showAvailableUpdateDialog(
	...
	const result = await showMessageBox(getMainWindow, {
		...
		message: `CamVerse ${version} is available.`,
...
async function showDownloadedUpdateDialog(
	...
	const result = await showMessageBox(getMainWindow, {
		...
		title: "Update Ready",
		message: isPreview
			? `CamVerse ${version} is ready to install.`
			: `CamVerse ${version} has been downloaded.`,
		detail: isPreview
...
export function setupAutoUpdates(
	...
	autoUpdater.on("update-available", (info) => {
		...
		setUpdateStatusSummary({
			status: "available",
			availableVersion: info.version,
			detail: `CamVerse ${info.version} is available.`,
		});
	...
	autoUpdater.on("update-not-available", () => {
		...
		setUpdateStatusSummary({
			status: "up-to-date",
			availableVersion: null,
			detail: `CamVerse ${app.getVersion()} is up to date.`,
		});
		clearVisibleUpdateToast(sendToRenderer);
		manualCheckRequested = false;
	});
	...
	autoUpdater.on("download-progress", (progress) => {
		...
		setUpdateStatusSummary({
			status: "downloading",
			availableVersion,
			detail: `Downloading CamVerse ${availableVersion}`,
		});
		writeUpdaterLog(
	...
	autoUpdater.on("update-downloaded", (info) => {
		...
		setUpdateStatusSummary({
			status: "ready",
			availableVersion: info.version,
			detail: `CamVerse ${info.version} is ready to install.`,
		});
		clearDeferredReminderTimer();
